...
# installed, running the frubyollowing commands should work:
...
module Jekyll
  module WordPress
    ...
    def self.process_post(post, db, options)
      ...
      podcast_match = content.match(/\[podcast\](.*)\[\/podcast\]/)
      if podcast_match
        podcast = podcast_match[1]
        categories << 'podcast'
        content = content.gsub(podcast_match[0],'')
      end
