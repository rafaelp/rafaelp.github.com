         ...
         $_GET['page'] == 'woothemes_image') 
    {
        ...
        if ( 'save' == $_REQUEST['action'] ) {
           ...
           foreach ($options as $key => $value) {
                ...
                update_option('woo_upload_errors',$errors);
...
<div class="wrap" id="woo_options">
    <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post"  enctype="multipart/form-data">
        ...
        <?php //Errors
        $upload_errors = get_option('woo_upload_errors');
        ...
        echo '<div style="clear:both;height:20px;"></div><div class="errors"><ul>' . "\n";
            ...
            foreach($upload_errors as $error)
            {
                 if($error['error'] == 'folder_not_created' && $error_shown != true){
                      echo '<li><strong>Oh No!</strong> We don\' have the permissions to create the upload folder on your server. Please create it manually: <em>/wp-content/<strong>woo_uploads</strong></em>. Thanks!</li>' . "\n";
                      $error_shown = true;
                }
        ...
        echo '</ul></div>' . "\n";
        delete_option('woo_upload_errors');
        } ?>
